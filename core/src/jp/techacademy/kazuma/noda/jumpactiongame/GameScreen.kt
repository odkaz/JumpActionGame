...
class GameScreen(private val mGame: JumpActionGame): ScreenAdapter() {
    ...
    private var mBlackhole: ArrayList<Blackhole>
    ...
    init {
        ...
        mBlackhole = ArrayList<Blackhole>()
        mGameState = GAME_STATE_READY
        mTouchPoint = Vector3()
        mSound = Gdx.audio.newSound(Gdx.files.internal("shock1.mp3"))
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.kazuma.noda.jumpactiongame")
    ...
    override fun render(delta: Float) {
        ...
        for (i in 0 until mBlackhole.size) {
            mBlackhole[i].draw(mGame.batch)
            mBlackhole[i].update(delta)
        }
    ...
    private fun createStage() {
        ...
        val blackHoleTexture = Texture("blackhole.png")
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (mRandom.nextFloat() > 0.6f) {
                ...
                star.setPosition(step.x + mRandom.nextFloat(), step.y + Star.STAR_HEIGHT)
                mStars.add(star)
            }

            if (mRandom.nextFloat() > 0.6f) {
                val black = Blackhole(blackHoleTexture, 0, 0, 480, 480)
                black.setPosition(mRandom.nextFloat() * WORLD_WIDTH, step.y + mRandom.nextFloat() * 5)
                mBlackhole.add(black)
            }
    ...
    private fun updatePlaying(delta: Float) {
        ...
        if (Gdx.input.isTouched) {
            ...
            val right = Rectangle(GUI_WIDTH / 2, 0f, GUI_WIDTH, GUI_HEIGHT)
    ...
    private fun checkCollision() {
        val playerBoundingRectangle = mPlayer.boundingRectangle

        if (playerBoundingRectangle.overlaps(mUfo.boundingRectangle)) {
            GAME_STATE_GAMEOVER
            return
        }
        ...
        for (i in 0 until mStars.size) {
            ...
            if (playerBoundingRectangle.overlaps(star.boundingRectangle)) {
                star.get()
                mScore++
        ...
        for (i in 0 until mBlackhole.size) {
            val black = mBlackhole[i]
            if (black.mState == Blackhole.BLACKHOLE_GONE) {
                continue
            }
            if (playerBoundingRectangle.overlaps(black.boundingRectangle)) {
                mSound.play()
        ...
        for (i in 0 until mSteps.size) {
            ...
            if (mPlayer.y > step.y) {
                if (playerBoundingRectangle.overlaps(step.boundingRectangle)) {
                    mPlayer.hitStep()
    ...
    private fun checkGameOver() {
        if (mPlayer.y < mHeightSoFar - CAMERA_HEIGHT / 2) {
            mGameState = GAME_STATE_GAMEOVER
